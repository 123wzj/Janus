...
class TrainingManager:
    ...
    def find_optimal_threshold(self, results):
        ...
        score_mean = valid_results['score'].mean()
        score_std = valid_results['score'].std()
        ...
        # 3. 生成更合理的候选阈值范围
        try:
            # 3.1 基于分位数的候选值（调整分位数范围）
            percentile_thresholds = [
                np.percentile(valid_results['score'], p)
                for p in [90, 92, 94, 95, 96, 97]  # 更细致的分位数范围
            ]

            # 3.2 基于统计的候选值（调整标准差范围）
            statistical_thresholds = [
                score_mean + i * score_std
                for i in [1.5, 1.75, 2.0, 2.25]  # 更合理的标准差倍数
            ]
            ...
            for threshold in candidates:
                ...
                if precision >= target_precision:
                    valid_thresholds.append({
                        ...
                        'f1': f1,
                        'g_mean': g_mean,
                        'score': f1 * 0.4 + recall * 0.3 + g_mean * 0.3  # 平衡的评分策略
                    })
            ...
            if valid_thresholds:
                ...
                # 如果没有满足要求的阈值，选择精确率最高的
                backup_threshold = np.percentile(valid_results['score'], 95)
            ...
            print(f"\n计算最优阈值时出错: {str(e)}")
            # 使用更保守的备选阈值
            backup_threshold = np.percentile(valid_results['score'], 96)
            print(f"使用96分位数作为备选阈值: {backup_threshold:.4f}")
